        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">Perbarui data titik ukur grounding dan lampiran.</p>
                </div>
                <a href="{{ route('grounding-tests.show', $groundingTest) }}"
            ...
            <div class="bg-white border border-gray-100 rounded-2xl shadow-xl shadow-gray-200/40 overflow-hidden">
                <div class="p-8">
                    ...
                    <form method="POST" action="{{ route('grounding-tests.update', $groundingTest) }}" enctype="multipart/form-data" autocomplete="off">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div class="space-y-2">
                                ...
                                <x-text-input id="nama_pembuat" class="block w-full border-gray-200 focus:ring-cyan-500 focus:border-cyan-500 rounded-xl transition-all"
                                              ...
                                              :value="old('nama_pembuat', $groundingTest->nama_pembuat ?? $groundingTest->creator->name ?? '')"
                            ...
                            <div class="space-y-2 md:col-span-3">
                                ...
                                <textarea id="catatan" name="catatan" rows="3"
                                    ...
                                    placeholder="Tambahkan catatan bila perlu...">{{ old('catatan', $groundingTest->catatan) }}</textarea>
                                <x-input-error :messages="$errors->get('catatan')" />
                            </div>
                        </div>

                        @php
                            $existingItems = $groundingTest->items->keyBy('id');
                            $items = old('items');
                            if (!$items || !is_array($items) || count($items) === 0) {
                                $items = $groundingTest->items->map(fn ($item) => [
                                    'id' => $item->id,
                                            ...
                                            <tr class="border-t border-gray-100 block sm:table-row sm:border-0">
                                                ...
                                                <td class="px-4 py-3 block sm:table-cell">
                                                    ...
                                                    <div class="space-y-2">
                                                        ...
                                                        @if($existingItem && $existingItem->attachment_path)
                                                            <div class="mt-2 flex flex-col gap-3 rounded-lg border border-gray-100 bg-gray-50/70 p-2 sm:flex-row sm:items-center">
                                                                <img src="{{ Storage::url($existingItem->attachment_path) }}" alt="Lampiran" class="h-14 w-14 rounded-md border object-cover sm:h-12 sm:w-12">
                                                                <div class="flex-1 min-w-0">
                                                                    <p class="text-xs font-medium text-gray-600">Lampiran saat ini</p>
                                                                    <a href="{{ Storage::url($existingItem->attachment_path) }}" target="_blank" class="text-xs text-cyan-700 hover:text-cyan-800">Lihat ukuran penuh</a>
                                                                </div>
                                                                <label class="inline-flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-600">
                                                                    <input type="checkbox" name="items[{{ $index }}][delete_attachment]" value="1" class="h-4 w-4 rounded border-red-300 text-red-600 focus:ring-red-500">
                                                                    Hapus
                                                                </label>
                                                            </div>
                                                        @endif
                                                ...
                                                <td class="px-4 py-3 text-right block sm:table-cell">
                                                    ...
                                                    <button type="button" data-remove-row class="text-sm text-red-600 hover:text-red-700">
                                                        Hapus
                                                    </button>
                                                </td>
                                            </tr>
