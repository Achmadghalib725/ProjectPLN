...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 mb-4 sm:mb-6" x-data="{ showFilter: false }">
    ...
    <div class="p-4" :class="{ 'hidden sm:block': !showFilter }">
        <form method="GET" action="{{ route('gudang.stok.index') }}" class="flex flex-col sm:flex-row gap-3 sm:gap-4" data-ajax-form data-ajax-target="#stok-content">
            ...
            <div class="flex gap-2">
                ...
                @if(request()->hasAny(['search', 'status']))
                    <a href="{{ route('gudang.stok.index', ['tab' => 'dipinjamkan']) }}" class="px-4 py-2.5 sm:py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">
                        Reset
                    </a>
                @endif
            </div>
