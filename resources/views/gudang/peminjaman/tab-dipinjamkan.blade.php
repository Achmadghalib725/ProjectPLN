...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 mb-4 sm:mb-6" x-data="{ showFilter: false }">
    ...
    <div class="p-4" :class="{ 'hidden sm:block': !showFilter }">
        <form method="GET" action="{{ route('gudang.peminjaman.index') }}" class="flex flex-col sm:flex-row gap-3 sm:gap-4" data-ajax-form data-ajax-target="#peminjaman-content">
            <input type="hidden" name="tab" value="dipinjamkan">
            <div class="flex-1">
                <input type="text"
                       name="search"
            ...
            <div class="flex gap-2">
                ...
                @if(request()->hasAny(['search', 'status']))
                    <a href="{{ route('gudang.peminjaman.index', ['tab' => 'dipinjamkan']) }}" class="px-4 py-2.5 sm:py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">
                        Reset
                    </a>
                @endif
            </div>
