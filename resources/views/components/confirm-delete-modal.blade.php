...
<div
    x-data="{
        ...
        open(title, message, formAction, method = 'DELETE', confirmText = 'Hapus', confirmClass = null) {
            this.title = title;
            this.message = message;
            this.formAction = formAction;
            this.method = method || 'DELETE';
            this.confirmText = confirmText || 'Hapus';
            this.confirmClass = confirmClass || 'bg-red-600 hover:bg-red-500';
            this.show = true;
    ...
    x-on:open-delete-modal.window="open($event.detail.title, $event.detail.message, $event.detail.action, $event.detail.method || 'DELETE', $event.detail.confirmText || 'Hapus', $event.detail.confirmClass || null)"
    ...
    <div class="min-h-screen flex items-start justify-center pt-[15vh] sm:pt-[20vh]">
        ...
        <div class="p-6">
            ...
            <form x-ref="deleteForm" method="POST" class="hidden">
                @csrf
                <input type="hidden" name="_method" x-ref="methodInput" :value="method" :disabled="!method || method === 'POST'">
            </form>
            ...
            <div class="mt-6 flex justify-end gap-3">
                ...
                <button
                    type="button"
                    @click="submit()"
                    class="inline-flex items-center px-4 py-2 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest transition-colors"
                    :class="confirmClass"
                >
